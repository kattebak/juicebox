...
function buildPrefix(login) {
  return login
    ? `🧃 created on behalf of @${login}`
    : "🧃 created on behalf of an agent";
}
...
function rewriteArgs(args, key, prefix) {
  const out = [...args];
  let bodyIdx = -1;
  let bodyFileIdx = -1;
...
export function runGhAsBot(args, token, login) {
  const prefix = buildPrefix(login);
  const { inject, key } = shouldInjectBody(args);
  const finalArgs = inject ? rewriteArgs(args, key, prefix) : args;
